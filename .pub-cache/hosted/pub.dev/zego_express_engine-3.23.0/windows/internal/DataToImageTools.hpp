...
std::vector<uint8_t> ConvertHBITMAPToVector(HBITMAP hBitmap) {
    ...
    bi.biHeight = -bmp.bmHeight; // 负值表示从上到下的DIB
    bi.biPlanes = 1;
    bi.biBitCount = 32; // 强制转换为32位ARGB格式
    ...
    // 分配内存
    result.resize(dwBmpSize + sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER));
