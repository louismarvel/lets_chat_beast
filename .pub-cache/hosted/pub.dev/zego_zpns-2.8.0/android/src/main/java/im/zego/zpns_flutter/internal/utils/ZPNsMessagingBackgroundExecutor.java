 ...
 */
public class ZPNsMessagingBackgroundExecutor implements MethodCallHandler {
  ...
  public static void setCallbackDispatcher(long callbackHandle) {
    ...
    SharedPreferences prefs =
        context.getSharedPreferences(ZPNsMessagingUtils.SHARED_PREFERENCES_KEY, 0);
  ...
  public void onMethodCall(MethodCall call, @NonNull Result result) {
    ...
    try {
      if (method.equals("initializedBackgroundExecutor")) {
  ...
  public void executeDartCallbackInBackgroundIsolate(Intent intent, final CountDownLatch latch) {
    ...
    ParcelableMap parcelableMap = intent.getParcelableExtra(ZPNsMessagingUtils.EXTRA_REMOTE_MESSAGE);
    Map<String, Object> messageMap = parcelableMap.getMap();
    if (messageMap != null) {
      Log.d(TAG, "ZPNs Native invokeMethod");
      HashMap<String,Object> resultMap = new HashMap<String, Object>();
      resultMap.put("userCallbackHandle",getUserCallbackHandle());
      resultMap.put("message",messageMap);
      backgroundChannel.invokeMethod(
          "onThroughBackgroundMessage", resultMap, result);

      ZPNsLogWriter.writeLog(TAG, "send background message method channel function to dart, user callback handle: " + getUserCallbackHandle());
    } else {
      Log.e(TAG, "RemoteMessage instance not found in Intent.");
      ZPNsLogWriter.writeLog(TAG, "no background message to send");
    }
  }
  ...
  private long getUserCallbackHandle() {
    SharedPreferences prefs =
        ContextHolder.getApplicationContext()
            .getSharedPreferences(ZPNsMessagingUtils.SHARED_PREFERENCES_KEY, 0);
  ...
  private void initializeMethodChannel(BinaryMessenger isolate) {
    ...
    backgroundChannel =
        new MethodChannel(isolate, "zego_zpns_background");
