...
abstract class JobIntentService extends Service {
  ...
  static final class CompatWorkEnqueuer extends WorkEnqueuer {
    ...
    void enqueueWork(Intent work) {
      ...
      Log.d(TAG, "CompatWorkEnqueuer enqueue work: " + work);
  ...
  static final class JobWorkEnqueuer extends JobIntentService.WorkEnqueuer {
    ...
    JobWorkEnqueuer(Context context, ComponentName cn, int jobId) {
      ...
      mJobInfo = b.setMinimumLatency(0).setOverrideDeadline(1000).build();
      mJobScheduler =
          (JobScheduler)
    ...
    @Override
    void enqueueWork(Intent work) {
      Log.d(TAG, "JobScheduler enqueueing work: " + work);
  ...
  public void onCreate() {
    super.onCreate();
    Log.d(TAG, "JobIntentService CREATING: " + this);
  ...
  public int onStartCommand(@Nullable Intent intent, int flags, int startId) {
    ...
    synchronized (mCompatQueue) {
      ...
      Log.d(TAG, "ensureProcessorRunningLocked startId" + startId);
  ...
  public static void enqueueWork(
      ...
      boolean useWakefulService) {
    synchronized (sLock) {
      Log.d(TAG, "ZPNs native enqueueWork: ensureJobId");
  ...
  static WorkEnqueuer getWorkEnqueuer(
    ...
    if (we == null) {
      if (Build.VERSION.SDK_INT >= 26 && !useWakefulService) {
        ...
        Log.d(TAG, "ZPNs native new job work enqueuer!");
      } else {
        we = new CompatWorkEnqueuer(context, cn);
        Log.d(TAG, "ZPNs native new compat work enqueuer!");
      }
  ...
  void ensureProcessorRunningLocked(boolean reportStarted) {
    Log.d(TAG, "ensureProcessorRunningLocked mCurProcessor：" + mCurProcessor);
    if (mCurProcessor == null) {
