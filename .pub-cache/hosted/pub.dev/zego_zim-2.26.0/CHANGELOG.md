## 2.21.1
- Fixed an issue where the onConversationSyncStateChanged event callback became invalid after a login-logout-login sequence within the same instance lifecycle.
- Fixed an issue where the unread message count became abnormal in two scenarios:
        1.When receiving messages that are not counted as unread, and
        2.When messages are sent from other devices under the same account during multi-device login.

## 2.21.0
- Supports multi-device login in the room module. Multiple devices using the same UserID can now join the same room without being kicked out. For details, see Multi-device Login - Room Management.
- Supports targeted group messaging. When sending a group message through the ZIM server-side API, you can specify which group members can see the message. For details, see Targeted Group Message.
- Related callback: onGroupMessageReceived
- Supports session list sync status callback. After a user logs in to the ZIM service or reconnects after disconnection, the onConversationSyncStateChanged callback will notify the status of synchronizing the latest session list from the ZIM server. For details, see Get Conversation List - Listen for Sync Status.
- Related callback: onConversationSyncStateChanged
- Supports editing the searchable content (searchedContent) of custom messages (ZIMCustomMessage). For details, see Edit Message.
- Related method: editMessage
- Optimized OPPO push notification integration. In accordance with OPUSH classification rules, developers can now configure category and notify_level fields. For details, see OPPO Push Integration Guide.
- Optimized performance for fetching group and session lists, improving the loading speed.

## 2.20.0

- Supports editing the content and other properties of single and group chat messages

- Support developers to customize user status, suitable for login or other situations that require states other than online status (such as invisibility, listening to music, and talking).

- When a user experiences a disconnection while sending a message, the ZIM SDK automatically resends the message if the network recovers within 30 seconds. If the network recovers after 30 seconds, the user can re-send the message manually.

- For a room that is temporarily disconnected and reconnected, keep the room instead of destroying it directly.

## 2.19.0

- Added Composite message types that support multiple content items in a message, including text, images, audio, video, files, and custom messages. It can realize a variety of combination methods such as text + picture, text + video, text + audio, etc., flexibly meet the needs of different scenarios. For details, see Sending and Receiving Messages - Sending and Receiving Combined Messages.

- Image message added support for HEIC and HEIF formats, further expanding the compatible image types to meet more scenario requirements.

- ZIMImageMessage supports developers to pass in image width and height information such as original image, large image and thumbnail when sending image messages. ZIMVideoMessage allows developers to pass in the width and height of the first frame of the video when sending a video message.

- When the user has reached the maximum number of flagged sessions (1000 by default), ZIM's processing logic is optimized from the original error to automatically cancel the earliest flagged sessions, ensuring that the user can continue to add new flagged sessions without restriction.

- The maximum number of tokens you can have per session has been increased from 20 to 30.

- Group member list information is associated with the user database to ensure that the latest local user information is obtained.
