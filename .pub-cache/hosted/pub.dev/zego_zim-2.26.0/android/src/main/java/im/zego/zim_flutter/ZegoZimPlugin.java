...
public class ZegoZimPlugin implements FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler {
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        try {
            ...
            Throwable cause = e.getCause(); // 先获取底层异常
            StringWriter sw = new StringWriter();
            PrintWriter pw = new PrintWriter(sw);

            if (cause != null) {
                cause.printStackTrace(pw); // 打印底层异常堆栈
            } else {
                e.printStackTrace(pw); // 如果底层为空，打印 InvocationTargetException 本身
            }
            ...
            result.error(
                "1",
                "InvocationTargetException: " + (cause != null ? cause.toString() : e.toString()) +
                    "\nStackTrace:\n" + stackTrace,
                null);
